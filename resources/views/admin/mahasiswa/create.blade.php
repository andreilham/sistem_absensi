...
<div class="max-w-2xl mx-auto bg-white rounded-xl shadow-sm p-6">
    <form action="{{ route('admin.mahasiswa.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        @csrf
        <div>
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Label Wajah (sesuai model AI)</label>
            <input type="text" name="face_label" value="{{ old('face_label') }}" placeholder="Contoh: yahya"
                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 outline-none">
            <p class="text-xs text-gray-500 mt-1">Harus sama dengan label di model Teachable Machine</p>
        </div>
        <div>
        ...
        <div class="flex gap-3 pt-2">
            <button type="submit" class="px-6 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-lg font-medium">Simpan</button>
